...
namespace Application.Services.Implementations
{
    public sealed class TermService : ITermService
    {
        ...
        public async Task<List<TermResponse>> ListAsync(UserRole role, CancellationToken ct)
        {
            ...
            return terms.Select(TermMapper.ToResponse).ToList();
        }

        public async Task<List<TermResponse>> ListForGradingAsync(int teacherId, int subjectId, CancellationToken ct)
        {
            var terms = await _uow.Terms.ListForTeacherGradingAsync(teacherId, subjectId, ct);
        ...
        public async Task<List<TermResponse>> ListForTeacherExamsViewAsync(CancellationToken ct)
        {
            var terms = await _uow.Terms.ListAllAsync( ct);
            return terms.Select(TermMapper.ToResponse).ToList();
